<?php
	...
	if (is_array($nodeList) == true){ // If the nodeList is valid (is an array)
		if (is_array($metisCallbackData) == true){ // If the metisCallbackData is an array, it means the json_decode from decodeJsonFile was successful
			...
			if (isset($metisCallbackData["contentOrDestinationNodes"])){ // If there is content we sent to the callback or a list of nodes (for replication)
			...
			if ($callbackFileAction !== "rp"){ // If we won't be replicating files
				if (atlasui_string_check($callbackFileAction, array("r", "d")) !== false){ // If we are reading or deleting a file
					$fileIOReply = fileActionHandler($callbackNodeNum, $callbackFiles, $callbackFileAction); // Assign the fileActionHandler response to fileIOReply
				}
				else{ // If we are NOT reading or deleting a file (a.k.a creating or updating)
					$fileIOReply = fileActionHandler($callbackNodeNum, $callbackFiles, $callbackFileAction, $callbackContentOrDestinationNodes); // Assign the fileActionHandler response to fileIOReply. Note the difference with this call is we applied the content.
				}
			}
